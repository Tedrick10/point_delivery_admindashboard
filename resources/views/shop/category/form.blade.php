<x-master-layout>
    <div class="container-fluid pds-page-wrap pds-motion-enter">
        <?php $id = $id ?? null; ?>
        @if(isset($id))
            {{ html()->modelForm($data, 'PATCH', route('shop-category.update', $id))->attribute('enctype', 'multipart/form-data')->open() }}
        @else
            {{ html()->form('POST', route('shop-category.store'))->attribute('enctype', 'multipart/form-data')->open() }}
        @endif
        <div class="row">
            <div class="col-lg-12">
                <div class="card pds-page-card">
                    <div class="card-header pds-page-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('shop-category.index') }}" class="btn btn-sm btn-primary">{{ __('message.back') }}</a>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-3 col-lg-2 mb-4 mb-md-0">
                                @include('partials._form_image_upload', [
                                    ...
                                    'previewId' => 'category_image_preview',
                                    'inputId' => 'category_image_input',
                                    'title' => __('message.image'),
                                    'imageUrl' => (isset($data) && getMediaFileExit($data, 'category_image'))
                                        ? getSingleMedia($data, 'category_image')
                                ...
                                <div class="pds-form-section pds-form-section--last">
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-6 col-lg-4">
                                            ...
                                            {!! html()->text('name', old('name', optional($data ?? null)->name))->placeholder(__('message.name'))->class('form-control')->required() !!}
                                        </div>
                                        <div class="form-group col-md-6 col-lg-4">
                                            {!! html()->label('Display Group <span class="text-danger">*</span>')->class('form-control-label') !!}
                                            {!! html()->select('category_group', ['quick_link' => 'Quick Links (Row 1)', 'product_type' => 'Product Types (Row 2)'], old('category_group', optional($data ?? null)->category_group ?? 'product_type'))->class('form-control select2js')->required() !!}
                                        </div>
                                        <div class="form-group col-md-6 col-lg-4">
