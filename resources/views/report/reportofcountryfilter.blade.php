<div class="pds-report-toolbar">
    {!! html()->form('GET', route('report-of-country'))->id('filter-form')->class('pds-report-toolbar__form')->open() !!}
        <div class="pds-report-toolbar__field pds-report-toolbar__field--select">
            {!! html()->label(__('message.country') . ' <span class="text-danger">*</span>')->for('country_id')->class('pds-report-toolbar__label') !!}
            <span class="text-danger d-block small" id="form_validation_country_id"></span>
            {!! html()->select('country_id', $selectedCountry, request('country_id'))
        ...
        <div class="pds-report-toolbar__actions">
            ...
            <a href="{{ route('report-of-country') }}" class="btn btn-sm btn-outline-primary">
...
<script>
    $(document).ready(function() {
        $('#export-button').on('click', function(e) {
            if (!$('#filter-form select[name="country_id"]').val()) {
                e.preventDefault();
                $('#form_validation_country_id').text("{{ __('message.country_required') }}");
            } else {
                $('#form_validation_country_id').text('');
            }
        });
    });
</script>
