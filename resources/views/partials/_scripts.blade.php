...
if (window.PDS_MYANMAR_NRC_DATA_URL) {
    $.getJSON(window.PDS_MYANMAR_NRC_DATA_URL).done(function (data) {
        window.PDS_MYANMAR_NRC_DATA = data;
        if (window.PdsMyanmarNrc) {
            window.PdsMyanmarNrc.data = data;
            if (typeof window.PdsMyanmarNrc.autoInitVisibleBoxes === 'function') {
                window.PdsMyanmarNrc.autoInitVisibleBoxes();
            }
        }
    });
}
