    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.loadRemoteModel', function(e) {
            ...
            if (url.indexOf('#') == 0) {
                ...
                $.get(url, function(data) {
                    $('#remoteModelData').html(data);
                    $('#remoteModelData').find('script').each(function () {
                    ...
                    if (typeof window.initOsAccountModal === 'function') {
        ...
        window.pdsNotificationCategory = 'all';

        function pdsNotificationElements() {
            var $toggle = $('#pdsNotificationDropdownToggle');
            return {
                $toggle: $toggle,
                $dropdown: $toggle.closest('.pds-notify-dropdown'),
                $menu: $toggle.closest('.pds-notify-dropdown').find('.pds-notify-menu')
            };
        }

        function pdsIsNotificationOpen() {
            var els = pdsNotificationElements();
            return els.$menu.hasClass('show');
        }

        function pdsSetNotificationOpen(shouldOpen) {
            var els = pdsNotificationElements();
            if (shouldOpen) {
                els.$menu.addClass('show');
                els.$dropdown.addClass('show');
                els.$toggle.attr('aria-expanded', 'true');
                return;
            }

            els.$menu.removeClass('show');
            els.$dropdown.removeClass('show');
            els.$toggle.attr('aria-expanded', 'false');
        }

        window.updateNotificationDropdownMeta = function(unreadCount, unreadText) {
            var count = Number(unreadCount) || 0;
            if (unreadText) {
                $('.pds-notify-unread-text').text(unreadText);
            }
            $('.pds-notify-header-count').text(count);
            if (count > 0) {
                $('.pds-notify-mark-read').removeClass('d-none');
            } else {
                $('.pds-notify-mark-read').addClass('d-none');
            }
        };

        window.notificationList = function(type, category) {
            type = type || '';
            if (category) {
                window.pdsNotificationCategory = category;
            } else {
                category = window.pdsNotificationCategory || 'all';
            }
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    if (typeof res.unread_count !== 'undefined') {
                        window.updateNotificationDropdownMeta(res.unread_count, res.unread_text);
                    }
        ...
        window.pdsFilterNotifications = function(category, el, event) {
            if (event) {
            ...
            window.pdsNotificationCategory = category;
            $('.pds-notify-menu .pds-notification-tab').removeClass('is-active');
            if (el) {
                $(el).addClass('is-active');
            } else {
                $('.pds-notify-menu .pds-notification-tab[data-category="' + category + '"]').addClass('is-active');
            }
            window.notificationList('', category);
            return false;
        };

        window.pdsCloseNotificationDropdown = function(event) {
            if (event) {
                event.preventDefault();
                event.stopPropagation();
            }
        ...
        window.pdsToggleNotificationDropdown = function(event) {
            if (event) {
                event.preventDefault();
                event.stopPropagation();
            }

            if (pdsIsNotificationOpen()) {
                pdsSetNotificationOpen(false);
                return false;
            }

            $('.pds-topbar .dropdown.show').not('.pds-notify-dropdown').each(function() {
                $(this).removeClass('show');
                $(this).find('.dropdown-menu').removeClass('show');
                $(this).find('[data-toggle="dropdown"]').attr('aria-expanded', 'false');
            });

            pdsSetNotificationOpen(true);
            window.notificationList('', window.pdsNotificationCategory || 'all');
            return false;
        };
        ...
        $(document).on('click','.notifyList',function(event)
        {
            event.preventDefault();
            event.stopPropagation();
            window.notificationList($(this).attr('data-type'), window.pdsNotificationCategory || 'all');
        });
        ...
        @if(isset($assets) && (in_array('phone', $assets) || in_array('contact_nbr', $assets)))
            $(document).ready(function(){
                ...
                var defaultCountryCode = currencyToCountryCode[currencyCode] || 'mm';

                inputs.forEach(function(input) {
                    if (!input || input.readOnly || input.hasAttribute('readonly') || input.hasAttribute('data-user-reg-phone')) {
                        return;
                    }
                    if(input) {
