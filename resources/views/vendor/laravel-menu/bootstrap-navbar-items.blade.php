@foreach($items as $item)
    ...
    <li @lm_attrs($item) @if($item->hasChildren()) @endif @lm_endattrs>
        @if($item->link) <a @lm_attrs($item->link)
            @if($item->hasChildren()) data-toggle="collapse" role="button" aria-expanded="{{ $active != '' ? 'true' : 'false' }}" aria-controls="collapseExample" @else class="nav-link" @endif @lm_endattrs href="{!! $item->url() !!}" title="{{ trim(preg_replace('/\s+/', ' ', strip_tags($item->title))) }}">
            {!! $item->title !!}
            @if($item->hasChildren())
