<x-master-layout :assets="$assets ?? []">
        <div class="container-fluid pds-page-wrap pds-order-detail pds-motion-enter">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card pds-page-card pds-order-header-card pds-order-animate">
                        <div class="card-body pds-order-header">
                            <div class="pds-order-header-title">
                                <p class="pds-order-eyebrow mb-1">{{ __('message.order') }} #{{ $id }}</p>
                                <h4 class="pds-page-title mb-0">{{ __('message.order_detail') }}</h4>
                            </div>
                            <div class="pds-order-header-actions">
                                ...
                                @if(Auth::user()->user_type == 'admin' && $data['status'] == 'completed' && $complate_data == null)
                                    <button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#returnOrderModal">
                                        <i class="fa fa-undo"></i> {{ __('message.return_order') }}
                                    </button>
                                @endif

                                @if(Auth::user()->user_type == 'admin')
                                    <a href="{{ route('order.index') }}" class="btn btn-sm btn-primary"><i class="fa fa-angle-double-left"></i> {{ __('message.back') }}</a>
                                @else
                                    <a href="{{ route('home') }}" class="btn btn-sm btn-primary"><i class="fa fa-angle-double-left"></i> {{ __('message.back') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            @php $isPhotoOrderAdmin = (int) ($data['is_photo_order'] ?? 0) === 1 && (Auth::user()->user_type === 'admin' || Auth::user()->hasRole(['admin', 'demo_admin'])); @endphp
            @if($isPhotoOrderAdmin)
            <div class="row pds-order-blocks">
                @include('order.photo-order-panel', ['data' => $data, 'photoOrderImages' => $photoOrderImages ?? collect()])
            </div>
            @endif
            @if(Auth::user()->user_type === 'admin' || Auth::user()->hasRole(['admin', 'demo_admin']))
            <div class="row pds-order-blocks">
                @include('order.order-chat-panel', ['data' => $data, 'orderChatSupport' => $orderChatSupport ?? null])
            </div>
            @endif
            <div class="row pds-order-blocks">
                <div class="col-lg-12">
                    <div class="row pds-order-layout">
                        <div class="col-lg-8 col-xl-8 pds-order-main">
                            <div class="card pds-page-card pds-order-main-card">
                                ...
                                <div class="card-body pb-0 pt-0">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <div class="card card-block pds-order-address-card">
                                                <div class="col-md-12">
                                    ...
                                    @if((int) ($data['is_photo_order'] ?? 0) !== 1 && $data->packaging_symbols != null)
                                ...
                                @if(!empty($mediaItems['prof_file']) && (int) ($data['is_photo_order'] ?? 0) !== 1)
                                ...
                                @endif
                                @if($data['status'] == 'pickup_error')
                                    <div class="card-body mt-0 pt-0 pb-0">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <h4 class="card-title mb-2 font-weight-bold">{{ __('message.pickup_error') }}</h4>
                                                <div class="card card-block">
                                                    <div class="col-md-12 mb-3 bg-dark-danger">
                                                        <h5 class="text-danger mt-2">{{ $data->reason ?: '-' }}</h5>
                                                        @if(!empty($data->pickup_error_at))
                                                            <p class="text-muted mb-2">{{ dateAgoFormate($data->pickup_error_at) }}</p>
                                                        @endif
                                                        @if(!empty($data->pickup_error_choice))
                                                            <p class="mb-2"><strong>{{ __('message.user_pickup_error_choice') }}:</strong> {{ pickupErrorChoiceLabel($data->pickup_error_choice) }}</p>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                @endif
                                @if($data['reason'] != null && $data['status'] != 'cancelled' && $data['status'] != 'pickup_error')
                                    <div class="card-body mt-0 pt-0 pb-0">
                                        <div class="row">
                                            <div class="col-md-12">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @if(!empty($data->extra_charges))
                                                            @php
                                                                ...
                                                                $totall = $data->fixed_charges + $data->distance_charge + $data->weight_charge + $data->vehicle_charge + $data->insurance_charge - $data->discount_amount;
                                                        ...
                                                        <tr>
                                                                ...
                                                                if (isset($data->extra_charges))
                                                                {
                                                                    ...
                                                                    $total_amount = $data->fixed_charges + $data->distance_charge + $data->weight_charge + $data->vehicle_charge + $data->insurance_charge + $totalextracharge - $data->discount_amount;

                                                                } else {
                                                                    $total_amount = $data->fixed_charges + $data->distance_charge + $data->weight_charge + $data->vehicle_charge + $data->insurance_charge - $data->discount_amount;
                                                                }
                                                        ...
                                                        @if($data['status'] == 'cancelled')
                                                            @php
                                                                ...
                                                                $original_total_amount = $data->fixed_charges + $data->distance_charge + $data->weight_charge + $data->vehicle_charge + $data->insurance_charge + $totalextracharge - $data->discount_amount;
                        ...
                        <div class="col-lg-4 col-xl-4 pds-order-aside">
                            @if(auth()->user()->user_type != 'client')
                                <div class="card pds-page-card pds-order-user-card">
                                    <div class="card-body">
                                        <h4 class="pds-section-title mb-0"> {{ __('message.about_user') }}</h4>
                                        <hr class="mt-2 mb-3">
                                        <div class="mt-2">
                                            <div class="row">
                                                <div class="col-md-4">
                                        ...
                                        <div class="mt-2">
                                            <div class="row">
                                                ...
                                                <div class="col-md-8">
                                                    ...
                                                    <h6 class="mt-1">{{ maskSensitiveInfo('contact_number', optional($data->delivery_man)->riderAssignedPhone() ?: optional($data->delivery_man)->contact_number) }}</h6>
                                                    <div class="mt-1">
                            ...
                            @endif
                            <div class="card pds-page-card pds-order-history-card">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <h4 class="pds-section-title mb-2">{{ __('message.orderhistory') }}</h4>
                                            <hr class="mt-0 mb-3">
                                            @if(count($data->orderhistory) > 0)
                                                <div class="pds-order-timeline">
                                                    <ul class="pds-order-timeline-list">
                                                        @foreach($data->orderhistory as $history)
                                                            ...
                                                            @endphp
                                                            <li class="pds-order-timeline-item">
                                                                <div class="pds-order-timeline-dot" aria-hidden="true">
                                                                    <i class="{{ $iconClass }}"></i>
                                                                </div>
                                                                <div class="pds-order-timeline-content">
                                                                    <div class="pds-order-timeline-head">
                                                                        <h6 class="pds-order-timeline-title">{{ __('message.' . $history->history_type) }}</h6>
                                                                        <small class="pds-order-timeline-time">{{ dateAgoFormate($history->created_at) }}</small>
                                                                    </div>
                                                                    <p class="pds-order-timeline-message">
                                                                        {{ $history->history_message }}
                                                                        @if($deliveryManId)
                                                                            <a href="{{ route('deliveryman-view.show', $deliveryManId) }}" class="pds-order-timeline-link">{{ __('message.view_more') }}</a>
                                                                        @endif
                                                                    </p>
                                                                </div>
                                                            </li>
        ...
        <div class="modal fade" id="returnOrderModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    {!! html()->form('POST', route('order.create-return', $data->id))->open() !!}
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('message.return_order') }}</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            {!! html()->label(__('message.return_reason') . ' *')->class('form-control-label') !!}
                            {!! html()->textarea('reason', null)->class('form-control')->rows(3)->required() !!}
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('message.cancel') }}</button>
                        <button type="submit" class="btn btn-warning">{{ __('message.return_order') }}</button>
                    </div>
                    {!! html()->form()->close() !!}
                </div>
            </div>
