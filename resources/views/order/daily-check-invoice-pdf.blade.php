...
<body>
    <div class="header">
        ...
        <div class="meta">
            {{ __('message.invoice_no') }} {{ $invoice->invoice_no }}
            @if(!empty($remittedOnly) && $invoice->remitted_date)
                | {{ __('message.remitted_date') }}: {{ $invoice->remitted_date->format('d-m-Y') }}
            @endif
        </div>
        <div class="meta">{{ __('message.sender') }}: {{ $partyName }}</div>
    </div>
    ...
    <table>
        ...
        <tbody>
            @foreach($slipRows as $index => $row)
                <tr>
                    ...
                    <td class="text-right">{{ $row['deli_amount_display'] }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td class="text-right">{{ number_format($slipTotals['deli_amount']) }}</td>
    ...
    @if(!empty($remittedOnly) && $invoice->remitted_photo_path)
        <div style="margin-top:18px;">
            <strong>{{ __('message.remitted_photo') }}</strong>
            @php
                $photoPath = \Illuminate\Support\Facades\Storage::disk('public')->path($invoice->remitted_photo_path);
            @endphp
            @if(file_exists($photoPath))
                <div style="margin-top:8px;">
                    <img src="{{ $photoPath }}" style="max-width:420px;max-height:420px;" alt="Remitted Photo">
                </div>
            @endif
        </div>
    @endif
