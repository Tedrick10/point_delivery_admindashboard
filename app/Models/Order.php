...
class Order extends Model implements HasMedia
{
    ...
    protected $casts = [
        ...
        'vehicle_id' => 'integer',
        'pickup_error_at' => 'datetime',
        'pickup_error_choice_at' => 'datetime',
        // 'pickup_point' => 'array',
        // 'delivery_point' => 'array',
    ];

    public function pickupErrorWindow(): array
    {
        return pickupErrorClientWindow($this);
    }
    ...
    {
        return $this->hasMany(DispatchOrderItem::class, 'order_id', 'id');
    }

    public function orderChatMessages()
    {
        return $this->hasMany(OrderChatMessage::class, 'order_id');
    }
    ...
    {
        $user = auth()->user();
        $isHub = function_exists('isDispatchHub') && isDispatchHub($user);
        if (in_array($user->user_type, ['admin', 'super_admin'], true) || $isHub) {
            $branchId = function_exists('forcedBranchId') ? forcedBranchId($user) : null;
            if ($branchId) {
                return $query->where(function ($outer) use ($branchId) {
                    $outer->whereHas('client', function ($client) use ($branchId) {
                        $client->where('branch_id', $branchId);
                    })->orWhereHas('dispatchItems', function ($q) use ($branchId) {
