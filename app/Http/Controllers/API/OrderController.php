...
class OrderController extends Controller
{
    ...
    {
        $order = Order::myOrder();
        $skipLocationFilter = $request->filled('delivery_man_id')
            || (auth()->check() && optional(auth()->user())->user_type === 'delivery_man');
        ...
        $order->when(request('delivery_man_id'), function ($q) {
             return $q->where('delivery_man_id', request('delivery_man_id'));
        });

        $order->when(request('country_id') && ! $skipLocationFilter, function ($q) {
            return $q->where('country_id', request('country_id'));
        });

        $order->when(request('city_id') && ! $skipLocationFilter, function ($q) {
        ...
        if (request('status') === 'courier_assigned') {
            ...
            $order = Order::query()
                ...
                ->whereIn('status', ['courier_assigned', 'create'])
                ->whereNotIn('status', ['courier_picked_up', 'courier_departed', 'completed', 'cancelled'])
                ->orderBy('id', 'desc');
        }

        if (request('status') === 'courier_picked_up') {
            $authUser = auth()->user();
            $order = Order::query()
                ->where('delivery_man_id', $authUser->id)
                ->whereIn('status', ['courier_picked_up', 'completed'])
                ->orderBy('id', 'desc');
        }

        if (request('status') === 'courier_departed') {
            $authUser = auth()->user();
            Order::where('delivery_man_id', $authUser->id)
                ->where('status', 'reschedule')
                ->whereDate('rescheduledatetime', today())
                ->update(['status' => 'courier_departed']);

            $order = Order::where('delivery_man_id', $authUser->id)->where('status', 'courier_departed')->orderBy('id', 'desc');
        }
        if (request('status') === 'courier_pending') {
            $authUser = auth()->user();
            ...
            $order = Order::where('status', 'create')
                ...
                ->whereRaw("
                    6371 * acos(
                        cos(radians(?))
                        * cos(radians(JSON_UNQUOTE(JSON_EXTRACT(pickup_point, '$.latitude'))))
                        * cos( radians(JSON_UNQUOTE(JSON_EXTRACT(pickup_point, '$.longitude'))) - radians(?) )
                        + sin(radians(?))
                        * sin(radians(JSON_UNQUOTE(JSON_EXTRACT(pickup_point, '$.latitude'))))
                    ) <= ? ", [
                    $authUser->latitude,
                    $authUser->longitude,
                    $authUser->latitude,
        ...
        // User App: dedicated Pick Up Cancelled list vs main day-by-day list.
        if (request('pickup_cancelled') == '1' || request('pickup_cancelled') === 1) {
            $order = $order->where('status', 'cancelled')
                ->where(function ($q) {
        ...
        } elseif ($request->filled('client_id') && ! $request->filled('status')) {
            // Main client pick-up list hides Pick Up Cancelled orders.
            $order = $order->where('status', '!=', 'cancelled');
        }

        // User App day-by-day pickup list: filter by received day (pickup_datetime).
        if (request('pickup_from_date') != null && request('pickup_to_date') != null) {
            $pickupFrom = request('pickup_from_date');
            $pickupTo = request('pickup_to_date');
            $order = $order->where(function ($dateQuery) use ($pickupFrom, $pickupTo) {
                $dateQuery->where(function ($q) use ($pickupFrom, $pickupTo) {
                    $q->whereDate('pickup_datetime', '>=', $pickupFrom)
                        ->whereDate('pickup_datetime', '<=', $pickupTo);
                })->orWhere(function ($fallback) use ($pickupFrom, $pickupTo) {
                    $fallback->whereNull('pickup_datetime')
                        ->whereDate('created_at', '>=', $pickupFrom)
                        ->whereDate('created_at', '<=', $pickupTo);
                });
            });
    ...
    {
        $id = $request->id;
        $order = Order::where('id', $id)->withTrashed()->with([
            'delivery_man',
            'profofPictures',
            'dispatchItems' => function ($query) {
                $statuses = app(\App\Services\DispatchOrderWorkflowService::class)->clientVisibleItemStatuses();
    ...
    public function calculateTotal(Request $request)
    {
        ...
        $welcomeDiscount = 0;
        $welcomeInfo = ['eligible' => false, 'remaining' => 0];
        if (auth()->check()) {
            $welcomeInfo = \App\Helpers\WelcomePromotionHelper::getDiscountForUser(auth()->user(), $totalAmountAll);
            $welcomeDiscount = $welcomeInfo['discount'] ?? 0;
            $totalAmountAll = max(0, $totalAmountAll - $welcomeDiscount);
        }
