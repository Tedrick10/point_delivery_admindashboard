...
class HomeController extends Controller
{
    ...
    {
        $languageOption = AppSetting::first()?->language_option ?? [config('app.locale', 'en')];
        $availableLocales = collect(languagesArray($languageOption))->pluck('id')->all();
    ...
    public function changeStatus(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'branch':
                $branch = \App\Models\Branch::find($request->id);
                $status = $request->status == 0 ? '0' : '1';
        ...
        if ($message_form != null) {
            $message = __('message.add_form_title', ['form' => $message_form]);
            if ($request->status == 0) {
                $message = __('message.remove_file_title', ['form' => $message_form]);
            }
        }
    ...
    public function getAjaxList(Request $request)
    {
        ...
        switch ($request->type) {
            ...
            case 'city-list';
                $items = City::select('id', 'name as text')->where('status', 1)->whereNull('deleted_at');
                if ($value != '') {
                    $items->where('name', 'LIKE', '%' . $value . '%');
                }
                $items = $items->get();
            ...
            case 'extra_charge_city':
                ...
                $items = City::select('id', 'name as text')->where('country_id', $classIds)->where('status', 1);
                if ($value !== '') {
                    $items->where('name', 'LIKE', '%' . $value . '%');
                }
                $items = $items->get();
            ...
            case 'country_base_city':
                $classIds = request('country_id');
                $items = City::select('id', 'name as text', 'fixed_charges', 'per_weight_charges', 'per_distance_charges', 'min_weight', 'min_distance')->where('country_id', $classIds)->where('status', 1);
                if ($value !== '') {
                    $items->where('name', 'LIKE', '%' . $value . '%');
                }
            ...
            case 'deliveryman_name':
                ...
                $items = $items->get()->map(function ($user) {
                    return [
                        'id' => $user->id,
                        'text' => $user->text,
                        'name' => $user->text,
            ...
            case 'dispatch_deliveryman_search':
                ...
                $items = $items->orderBy('name')->get()->map(function ($user) {
                    return [
                        'id' => $user->id,
                        'text' => $user->text,
                        'name' => $user->text,
            ...
            case 'os_dispatch_search':
                ...
                if (!$request->filled('id') && ($request->boolean('list_all') || $value === null || $value === '')) {
                    $items = $items->limit(200);
                } else {
                    $items = $items->limit(50);
                }
                $items = $items->get()->map(function ($user) {
                    $city = \App\Models\City::find($user->city_id ?? null);
                    $displayName = $user->text . ($city && $city->name ? ' (' . $city->name . ')' : '');
                ...
                break;
            case 'dispatch_customer_phone':
                $phones = collect();

                $users = User::select('contact_number', 'name', 'address')
                    ->whereNotNull('contact_number')
                    ->where('contact_number', '!=', '');
                if ($value != '') {
                    $users->where('contact_number', 'LIKE', '%' . $value . '%');
                }
                $phones = $phones->merge($users->limit(20)->get()->map(function ($user) {
                    return [
                ...
                $itemsQuery = \App\Models\DispatchOrderItem::select('customer_phone', 'customer_name', 'customer_address')
                    ->whereNotNull('customer_phone')
                    ->where('customer_phone', '!=', '');
                if ($value != '') {
                    $itemsQuery->where('customer_phone', 'LIKE', '%' . $value . '%');
                }
                $phones = $phones->merge($itemsQuery->limit(20)->get()->map(function ($row) {
                    return [
                ...
                $items = $phones->unique('id')->values()->take(30);
                break;
            case 'document_name':
    ...
    public function destroySelected(Request $request)
    {
        ...
        switch ($types) {
            ...
            case 'deliveryman-checked':
                foreach ($checked_ids as $id) {
                    ...
                    if ($user) {
                        ...
                        \App\Models\DeliveryManDocument::where('delivery_man_id', $user->id)->delete();
                        $user->forceDelete();
                    }
            ...
            case 'branch-checked':
                foreach ($checked_ids as $id) {
                    $branch = \App\Models\Branch::withTrashed()->where('id', $id)->first();
                    if ($branch) {
                        if (env('APP_DEMO')) {
                            $message = __('message.demo_permission_denied');
                            if (request()->ajax()) {
    ...
    public function ordermaplocation(Request $request)
    {
        ...
        $assets = ['location'];
        $selectedStatus = $request->get('status_filter', 'all');
        ...
        if (!empty($selectedCountryId)) {
            $ordersQuery->where('country_id', $selectedCountryId);
        }
        if (!empty($selectedCityId)) {
            $ordersQuery->where('city_id', $selectedCityId);
        }
    ...
    public function changePassword(Request $request)
    {
        ...
        if ($user == "") {
            ...
            redirect()->route('passwordpage')->withSuccess($message);
        }
