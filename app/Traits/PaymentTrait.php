...
trait PaymentTrait {
    ...
    /**
     * Cancelled after pickup_error — no Order Cancel Charge.
     */
    protected function isPickupErrorRelatedCancel(?Order $order): bool
    {
        if (! $order) {
            return false;
        }
    ...
    /**
     * Reverse any order_cancel_charge already applied for this order.
     */
    protected function refundPickupErrorCancelChargeIfAny(Order $order): void
    {
        $histories = WalletHistory::query()
            ->where('order_id', $order->id)
            ->where('transaction_type', 'order_cancel_charge')
            ->where('type', 'debit')
            ->get();
        ...
        $clientWallet = Wallet::firstOrCreate(['user_id' => $order->client_id]);
        $adminId = User::admin()->value('id');
        $adminWallet = $adminId ? Wallet::firstOrCreate(['user_id' => $adminId]) : null;

        foreach ($histories as $history) {
            $alreadyRefunded = WalletHistory::query()
                ->where('order_id', $order->id)
                ->where('transaction_type', 'order_cancel_charge_refund')
                ->where('type', 'credit')
                ->where('amount', $history->amount)
                ->exists();
            ...
            $amount = (float) ($history->amount ?? 0);
            if ($amount <= 0) {
                continue;
            }

            $clientWallet->increment('total_amount', $amount);
            WalletHistory::create([
                'user_id' => $order->client_id,
                'type' => 'credit',
                'currency' => $currency,
                'transaction_type' => 'order_cancel_charge_refund',
                'amount' => $amount,
                'balance' => $clientWallet->total_amount,
                'order_id' => $order->id,
                'datetime' => date('Y-m-d H:i:s'),
                'data' => [
                    'payment_id' => data_get($history->data, 'payment_id'),
                    'note' => 'Refund cancel charge for pickup error cancellation',
                ],
            ]);

            if ($adminWallet) {
                $adminWallet->decrement('total_amount', $amount);
                WalletHistory::create([
                    'user_id' => $adminId,
                    'type' => 'debit',
                    'currency' => $currency,
                    ...
                    'datetime' => date('Y-m-d H:i:s'),
                    'data' => [
                        'note' => 'Reverse cancel charge for pickup error cancellation',
                    ],
                ]);
            }
        }
    ...
    public function walletTransactionCancelled($order_id)
    {
        ...
        // Pick Up Error / Pick Up Cancelled: never deduct Order Cancel Charge.
        if ($this->isPickupErrorRelatedCancel($order)) {
            $this->refundPickupErrorCancelChargeIfAny($order);
