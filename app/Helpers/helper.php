...
/**
 * UI tip shown in User App only — must never be persisted as order/item remark.
 */
function autoOrderRemarkTipMessages(): array
{
    return [
        'ပါဆယ်အရမ်းကြီးပါက ကားဖြင့်ခေါ်ဆောင်နိုင်သည်',
        'ပါဆယ်အရမ်းကြီးပါက ကားဖြင့် ခေါ်ဆောင်နိုင်ပါသည်',
        'ပါဆယ်သေးပါက ကားဖြင့်ခေါ်ဆောင်ရန် ဆိုတဲ့ ခလုတ်ကို နှိပ်ရန် မလို။ ပါဆယ်အရမ်းကြီးမှသာ ကားဖြင့်ခေါ်ဆောင်ရန် ဆိုတဲ့ ခလုတ်ကို နှိပ်ရန်။',
        'ပါဆယ်သေးပါကကားဖြင့်ခေါ်ဆောင်ရန်ဆိုတဲ့ ခလုတ်ကိုနှိပ်ရန်မလိုပါ။ ပါဆယ်အရမ်းကြီးမှသာ ကားဖြင့်ခေါ်ဆောင်ရန်ဆိုတဲ့ခလုတ်ကိုနှိပ်ရန်။',
    ];
}

function stripAutoOrderRemarkTip(?string $value): string
{
    ...
    foreach (autoOrderRemarkTipMessages() as $tip) {
        if ($text === $tip) {
            return '';
        }
    ...
    // Collapse leftover blank lines after removing the tip.
    $text = preg_replace("/[ \t]*\n[ \t]*\n+/u", "\n", $text) ?? $text;
...
/**
 * Human-readable label for client pickup_error_choice.
 */
function pickupErrorChoiceLabel(?string $choice): ?string
{
    ...
    return match ($choice) {
        'cancel' => 'User Cancel သွားပါသည်။',
        'express' => 'အမြန်နှင့်အပ်ထားပါသည်။',
        'next_day' => 'နောက်ရက်မှ လာယူရန်။',
        'reorder' => 'ORDER ထပ်အပ်မယ်',
        default => $choice,
    };
}

/**
 * Client choices available while an order is in pickup_error.
 *
 * Day 0 until 5:00 PM (Asia/Yangon): cancel | express | next_day
 * Day 0 after 5:00 PM until midnight: cancel | next_day
 * Next day 12:00 AM – 11:30 AM: cancel | reorder
 * After next-day 11:30 until auto-cancel (+2 days): cancel | reorder
 */
function pickupErrorExpressCutoff(?Carbon $errorAt, ?Carbon $now = null): Carbon
{
    $tz = 'Asia/Yangon';
    $errorAt = ($errorAt ?? Carbon::now($tz))->copy()->timezone($tz);

    return $errorAt->copy()->setTime(17, 0, 0);
}

function pickupErrorExpressAvailable(?Carbon $errorAt, ?Carbon $now = null): bool
{
    $tz = 'Asia/Yangon';
    $now = ($now ?? Carbon::now($tz))->copy()->timezone($tz);
    $errorAt = ($errorAt ?? $now)->copy()->timezone($tz);

    // Express only on the same calendar day as the pickup error, until 5:00 PM.
    if (! $now->isSameDay($errorAt)) {
        return false;
    }
...
function pickupErrorClientWindow(?Order $order): array
{
    ...
    if (! $order || (! $isPickupError && ! $isExpressCancelled)) {
        return [
            ...
            'pickup_error_choice_label' => null,
            'remark' => null,
        ];
    }
    ...
    $tz = 'Asia/Yangon';
    $now = Carbon::now($tz);
    $errorAt = $order->pickup_error_at
        ? Carbon::parse($order->pickup_error_at)->timezone($tz)
        : Carbon::parse($order->updated_at)->timezone($tz);
    $autoCancelAt = $errorAt->copy()->addDays(2);
    $choice = $choiceEarly;
    $choiceAt = $order->pickup_error_choice_at
        ? Carbon::parse($order->pickup_error_choice_at)->timezone($tz)
        : null;
    $choiceLabel = pickupErrorChoiceLabel($choice ?: null);

    $isErrorDay = $now->isSameDay($errorAt);
    $nextDayStart = $errorAt->copy()->addDay()->startOfDay();
    $nextDayCutoff = $errorAt->copy()->addDay()->setTime(11, 30, 0);
    $isNextDayMorning = $now->greaterThanOrEqualTo($nextDayStart)
        && $now->lessThanOrEqualTo($nextDayCutoff);

    $showExpressPhone = false;
    $expressMessage = null;
    $expressAvailable = pickupErrorExpressAvailable($errorAt, $now);
    // Express phone only on error day (never next day morning).
    if ($isErrorDay && $choice === 'express' && $choiceAt && $expressAvailable) {
        $showExpressPhone = $choiceAt->copy()->addHours(5)->greaterThan($now);
        if ($showExpressPhone) {
            $expressMessage = 'အမြန် SERVICE ကိုဆက်သွယ်လိုပါက အောက်ပါဖုန်းကို ခေါ်ဆိုပေးပါရန်';
        }
    }

    // Cancelled + express: phone box only (status is Pick Up Cancelled).
    if ($isExpressCancelled) {
        return [
            'status' => 'cancelled',
            'options' => [],
            ...
            'pickup_error_at' => $errorAt->toIso8601String(),
            'pickup_error_choice' => $choice,
            'pickup_error_choice_label' => $choiceLabel,
            'remark' => $order->reason,
        ];
    }

    if ($now->greaterThanOrEqualTo($autoCancelAt)) {
        return [
            'status' => 'pickup_error',
            'options' => [],
            'show_express_phone' => false,
            ...
            'auto_cancel_at' => $autoCancelAt->toIso8601String(),
            'pickup_error_at' => $errorAt->toIso8601String(),
            'pickup_error_choice' => $choice ?: null,
            'pickup_error_choice_label' => $choiceLabel,
            'remark' => $order->reason,
        ];
    }

    // After user already chose, hide further options (express keeps phone box only).
    if ($choice !== '') {
        return [
            'status' => 'pickup_error',
            'options' => [],
    ...
    if ($isErrorDay) {
        ...
        if ($expressAvailable) {
            $options[] = ['key' => 'express', 'label' => 'အမြန်နဲ့အပ်မလား'];
        }
        $options[] = ['key' => 'next_day', 'label' => 'နောက်နေ့အတွက်'];
    } elseif ($isNextDayMorning || $now->greaterThan($nextDayCutoff)) {
        // Next day 12:00 AM – 11:30 AM (and until auto-cancel): only CANCEL + reorder.
        $options = [
            ['key' => 'cancel', 'label' => 'CANCEL'],
            ['key' => 'reorder', 'label' => 'ORDER ထပ်အပ်မယ်'],
        ];
    }
...
/**
 * Best-effort cleanup of realtime tracking docs. Never fail the order API if Firebase is misconfigured.
 */
function deleteFirebaseDeliveryOrderDocument($orderId): void
{
    try {
        ...
        \Log::warning('Firebase order document cleanup skipped', [
            'order_id' => $orderId,
            'error' => $e->getMessage(),
        ]);
    }
...
/**
 * Same-day User App order cutoff (Asia/Yangon 11:30).
 *
 * After 11:30 AM until midnight → next-day / Pre Pick Up scheduling.
 * From 12:01 AM next day until 11:30 AM → normal same-day create again.
 */
function isSameDayOrderCutoffPassed(?Carbon $now = null): bool
{
    $tz = 'Asia/Yangon';
    $now = ($now ?? Carbon::now($tz))->copy()->timezone($tz);

    return $now->greaterThan($now->copy()->startOfDay()->setTime(11, 30, 0));
}
...
function nextDayOrderReceivedDatetime(?Carbon $now = null): string
{
    ...
    return $now->copy()->addDay()->startOfDay()->format('Y-m-d H:i:s');
}

/**
 * Yangon calendar day 00:00 for same-day client orders (before cutoff).
 */
function yangonTodayOrderReceivedDatetime(?Carbon $now = null): string
{
    $tz = 'Asia/Yangon';
    $now = ($now ?? Carbon::now($tz))->copy()->timezone($tz);
...
/**
 * Client API pickup day: today before cutoff, next day after cutoff.
 */
function resolveClientOrderPickupDatetime(?Carbon $now = null): string
{
    return isSameDayOrderCutoffPassed($now)
        ? nextDayOrderReceivedDatetime($now)
        : yangonTodayOrderReceivedDatetime($now);
}

/**
 * Admin dispatch list date bounds (Yangon calendar day).
 *
 * pickup_* are naive datetimes stored as Yangon wall clock.
 * created_* are UTC bounds for rows missing pickup_datetime.
 */
function parseDispatchFilterDateBounds(string $fromDmy, string $toDmy): array
{
    $tz = 'Asia/Yangon';
    $from = Carbon::createFromFormat('d-m-Y', $fromDmy, $tz)->startOfDay();
    $to = Carbon::createFromFormat('d-m-Y', $toDmy, $tz)->endOfDay();

    return [
        'pickup_from' => $from->format('Y-m-d H:i:s'),
        'pickup_to' => $to->format('Y-m-d H:i:s'),
        'created_from' => $from->copy()->utc()->format('Y-m-d H:i:s'),
        'created_to' => $to->copy()->utc()->format('Y-m-d H:i:s'),
    ];
}

/**
 * Format order/item timestamps for Admin dispatch UI (Yangon calendar day).
 */
function formatDispatchYangonDate($value): string
{
    if ($value === null || $value === '') {
        return '-';
    }
    ...
    try {
        return Carbon::parse($value)->timezone('Asia/Yangon')->format('d-m-Y');
    } catch (\Exception $e) {
        return '-';
    }
}
...
/**
 * Pre-orders become active on Order List from this datetime (next Yangon calendar day 00:00).
 */
function preOrderVisibleFromDatetime(?Carbon $now = null): string
{
    return nextDayOrderReceivedDatetime($now);
}

/**
 * Apply next-day scheduling after the same-day cutoff (client API creates only).
 */
function applySameDayOrderCutoffToOrderData(array $data, $status = null): array
{
    ...
    if (isSameDayOrderCutoffPassed()) {
        $data['pickup_datetime'] = nextDayOrderReceivedDatetime();
    } elseif (empty($data['pickup_datetime'])) {
        $data['pickup_datetime'] = yangonTodayOrderReceivedDatetime();
    }

    return $data;
}

function sanitizeOrderRemarkFields(array $data): array
{
    if (array_key_exists('description', $data)) {
        $data['description'] = stripAutoOrderRemarkTip($data['description'] ?? null);
    }

    foreach (['pickup_point', 'delivery_point'] as $pointKey) {
        if (! isset($data[$pointKey]) || ! is_array($data[$pointKey])) {
            continue;
        }
        foreach (['instruction', 'description', 'remark'] as $field) {
            if (array_key_exists($field, $data[$pointKey])) {
                $data[$pointKey][$field] = stripAutoOrderRemarkTip($data[$pointKey][$field] ?? null);
            }
        }
    }

    if (isset($data['dispatch_item']) && is_array($data['dispatch_item']) && array_key_exists('remark', $data['dispatch_item'])) {
        $data['dispatch_item']['remark'] = stripAutoOrderRemarkTip($data['dispatch_item']['remark'] ?? null);
    }
...
/**
 * Fingerprint of order list data for Admin live refresh polling.
 * Changes when users place orders, riders complete pickup, items change, etc.
 * Cached briefly so polling does not block PHP's single-threaded artisan serve.
 */
function adminOrderListLiveVersion(): string
{
    return (string) cache()->remember('admin_order_list_live_version', 2, function () {
        $orderCount = Order::query()->count();
        $orderMaxId = (int) (Order::query()->max('id') ?? 0);
        $orderUpdated = (string) (Order::query()->max('updated_at') ?? '');

        $itemCount = \App\Models\DispatchOrderItem::query()->count();
        $itemMaxId = (int) (\App\Models\DispatchOrderItem::query()->max('id') ?? 0);
        $itemUpdated = (string) (\App\Models\DispatchOrderItem::query()->max('updated_at') ?? '');

        $historyUpdated = (string) (OrderHistory::query()->max('updated_at') ?? '');
...
/**
 * Fingerprint of a single order's detail items for Admin live refresh.
 */
function adminOrderItemsLiveVersion(int $orderId): string
{
    $order = Order::query()->find($orderId);
    $orderUpdated = (string) ($order?->updated_at ?? '');
    $orderStatus = (string) ($order?->status ?? '');
    $totalParcel = (string) ($order?->total_parcel ?? '');

    $itemQuery = \App\Models\DispatchOrderItem::query()->where('order_id', $orderId);
    $itemCount = (clone $itemQuery)->count();
    $itemMaxId = (int) ((clone $itemQuery)->max('id') ?? 0);
    $itemUpdated = (string) ((clone $itemQuery)->max('updated_at') ?? '');
    $photoSum = (int) ((clone $itemQuery)->sum('photo_id') ?? 0);
    $weightSum = (string) ((clone $itemQuery)->sum('weight') ?? '0');
    // Include customer/pay fingerprints so User App item edits refresh Admin Detail List.
    $customerFingerprint = (string) ((clone $itemQuery)->selectRaw(
        "COALESCE(SUM(LENGTH(COALESCE(customer_name,''))+LENGTH(COALESCE(customer_phone,''))+LENGTH(COALESCE(customer_address,''))+LENGTH(COALESCE(remark,''))+LENGTH(COALESCE(item_name,''))+LENGTH(COALESCE(township,''))),0) as fp"
    )->value('fp') ?? '0');
    $payFingerprint = (string) ((clone $itemQuery)->selectRaw(
        'COALESCE(SUM(COALESCE(cust_get,0)+COALESCE(os_to_pay,0)+COALESCE(item_value,0)+COALESCE(deli_amount,0)+COALESCE(advance_paid,0)+COALESCE(os_paid,0)),0) as fp'
    )->value('fp') ?? '0');
...
function hiddenSidebarMenus(): array
{
    static $hiddenMenus = null;
    ...
    $value = \App\Models\Setting::query()
        ->where('type', 'sidebar')
        ->where('key', 'hidden_menus')
        ->value('value');

    $decoded = json_decode($value ?? '[]', true);

    $hiddenMenus = is_array($decoded) ? $decoded : [];

    return $hiddenMenus;
}

function isSidebarMenuVisible(string $menuKey): bool
{
    return !in_array($menuKey, hiddenSidebarMenus(), true);
}

/**
 * Detect this machine's LAN / WiFi IPv4 (not 127.0.0.1).
 * Used so mobile apps on the same WiFi can reach the admin API.
 */
function detectMachineLanIp(): ?string
{
    $candidates = [];

    $pushIfLan = static function (?string $ip) use (&$candidates): void {
    ...
    try {
        ...
        if ($sock !== false) {
            ...
            @socket_close($sock);
            $pushIfLan(is_string($ip) ? $ip : null);
        }
    } catch (\Throwable $e) {
        // ignore
    }
...
/**
 * Build http(s)://LAN_IP:port for mobile apps on the same WiFi.
 */
function detectMachineLanBaseUrl($request = null): string
{
    $request = $request ?: request();
    $scheme = $request->getScheme() ?: 'http';
    $port = (int) $request->getPort();
    $lanIp = detectMachineLanIp();
    ...
    $host = $isDefaultPort || $port <= 0 ? $lanIp : ($lanIp . ':' . $port);

    return $scheme . '://' . $host;
}

function setHiddenSidebarMenus(array $menuKeys): void
{
    \App\Models\Setting::updateOrCreate(
        ['type' => 'sidebar', 'key' => 'hidden_menus'],
        ['value' => json_encode(array_values(array_unique($menuKeys)))]
    );
}
...
{
    $relative = mediaPublicUrl($media, $conversionName);
...
function getSingleMedia($model, $collection = 'profile_image', $skip = true)
{
    ...
    if (getFileExistsCheck($media)) {
        ...
        switch ($collection) {
            case 'profile_image':
                $media = '/images/user/1.jpg';
                break;
            case 'site_logo':
                $media = '/images/logo.png';
                break;
            case 'site_dark_logo':
                $media = '/images/dark_logo.png';
                break;
            case 'gateway_image':
                $gateway_name = $model->type ?? 'default';
                $media = '/images/' . $gateway_name . '.png';
                break;
            case 'site_favicon':
...
function getSingleMediaCustomerSupport($model, $collection = 'support_image', $skip = true, $returnDefault = false)
{
    ...
    if (getFileExistsCheck($media)) {
        ...
        return $returnDefault ? '/images/default.png' : null;
    }
}
    ...
    if (count($attchments) > 0) {
        foreach ($attchments as $attchment) {
            if (getFileExistsCheck($attchment)) {
                array_push($files, mediaPublicUrl($attchment));
            }
        }
    }
...
function languagesArray($ids = [])
{
    $language = [
        ...
        ['title' => 'Burmese', 'id' => 'my', 'native' => 'မြန်မာ'],
        ...
        ['title' => 'Dutch', 'id' => 'nl'],
        ['title' => 'English', 'id' => 'en', 'native' => 'English'],
...
function languageFlagUrl(string $langId): string
{
    $localPath = public_path('images/flag/' . $langId . '.png');
    if (file_exists($localPath)) {
        return asset('images/flag/' . $langId . '.png');
    }
    ...
    if (!empty($countryMap[$langId])) {
        return 'https://flagcdn.com/w40/' . $countryMap[$langId] . '.png';
    }
...
{
    $prefixedKey = match ($key) {
        'email_verification' => str_starts_with($group, 'user') ? 'user_email_verification' : 'driver_email_verification',
        'mobile_verification' => str_starts_with($group, 'user') ? 'user_mobile_verification' : 'driver_mobile_verification',
        'document_verification' => 'driver_document_verification',
        default => $key,
    };

    $groupValue = SettingData($group, $prefixedKey);
    if ($groupValue !== null && $groupValue !== '') {
        return $groupValue;
    }

    return SettingData($key, $key);
}

function pushNotificationsEnabled(): bool
{
    $settings = appSettingData('get');
    $notificationSettings = $settings->notification_settings ?? [];

    if (!isset($notificationSettings['global']['IS_PUSH_NOTIFICATION_ENABLED'])) {
        return true;
    }
...
function classifyOrderNotificationCategory(string $historyType, ?User $actor = null): string
{
    $actor = $actor ?? auth()->user();

    if ($actor && isAdminPanelUser($actor)) {
        return 'admin_changes';
    }

    if ($actor && $actor->user_type === 'client') {
        return 'user_orders';
    }
...
function inferNotificationCategory(array $data): string
{
    ...
    if (in_array($type, ['create', 'draft', 'active', 'cancelled', 'bid_placed', 'payment_status_message'], true)) {
        return 'user_orders';
    }
...
function notificationCategoryLabel(string $category): string
{
    return match ($category) {
        ...
        default => __('message.notification_category_all'),
    };
}

function buildCustomerSupportNotificationPayload(CustomerSupport $support, User $sender, ?string $messageText = null, bool $isImage = false): array
{
    $isAdminSender = isAdminPanelUser($sender);
    $lineCount = 1;

    if (!$isImage && filled($messageText)) {
        $lineCount = max(1, count(preg_split('/\R+/', trim($messageText))));
    }

    if ($isAdminSender) {
        $summary = __('message.notification_admin_replied', [
            'admin' => $sender->name,
            'id' => $support->id,
        ]);
        $category = 'admin_changes';
    } elseif ($isImage) {
        $summary = __('message.notification_user_sent_image', ['user' => $sender->name]);
        $category = 'user_messages';
    } elseif ($lineCount === 1) {
        $summary = __('message.notification_user_sent_one_message', ['user' => $sender->name]);
        $category = 'user_messages';
    } else {
        $summary = __('message.notification_user_sent_messages', [
            'user' => $sender->name,
            'count' => $lineCount,
        ]);
        $category = 'user_messages';
    ...
    return [
        ...
        'subject' => __('message.customersupport.support_id', ['id' => $support->id]),
        'message' => $summary,
        'category' => $category,
...
function notificationRouteForData(array $data): ?string
{
    ...
    if (!empty($data['id']) && inferNotificationCategory($data) !== 'user_messages') {
        return route('order.show', $data['id']);
    }

    return null;
}

function notifyAdminsCustomerSupport(array $notification_data): void
{
    $payload = array_merge([
        'type' => 'customersupport',
        'category' => 'user_messages',
    ], $notification_data);
    $payload['type'] = 'customersupport';
    if (empty($payload['category'])) {
        $payload['category'] = inferNotificationCategory($payload);
    }

    getPanelStaffUsers()->each(function ($admin) use ($payload) {
        $admin->notify(new \App\Notifications\CustomerSupportNotification($payload));

        if (!empty($admin->player_id) && pushNotificationsEnabled()) {
            $pushData = $payload;
            $pushData['id'] = 'SUPPORT_' . $payload['support_id'];
            $pushData['subject'] = $payload['subject'] ?? __('message.customer_support');
...
function notifyUserCustomerSupport(User $user, array $notification_data): void
{
    ...
    if (!empty($user->player_id) && pushNotificationsEnabled()) {
        $pushData = $payload;
        $pushData['id'] = 'SUPPORT_' . $payload['support_id'];
        $pushData['subject'] = $payload['subject'] ?? __('message.customer_support');
        $user->notify(new CommonNotification('customersupport', $pushData));
    }
}
...
function saveOrderHistory($data)
{
    ...
    switch ($data['history_type']) {
        ...
        case 'create':
            ...
            if (!empty($deliveryContact) && !empty($parcelMessage)) {
                try {
                    ...
                    $smsService->sendSMS($deliveryContact, $parcelMessage);
        ...
        case 'cancelled':
            ...
            deleteFirebaseDeliveryOrderDocument($orderData->id);
            break;
        ...
        case 'courier_assigned':
            ...
            Order::updateOrCreate( ['id' => $data['order']->id], $datadatatime );

            /* ---------------- Email ---------------- */
            if ($setting == 1) {
                $dynamicData = [
                    '[order ID]'     => $data['order']->id,
                    '[status]'       => str_replace('_', ' ', ucfirst($data['order']->status)),
                    '[Company name]' => config('app.name'),
                ...
                if ($email && isset($emailData)) {
                    $mailDescription = str_replace(
                        array_keys($dynamicData),
                        array_values($dynamicData),
                        $emailData->mail_description ?? ''
                    );

                    Mail::to($email)->send(
                        new SendMail( $emailData->subject ?? '', $mailDescription,
                            [
                                'order_id'     => $data['order']->id,
                                'status'       => $data['order']->status,
                                'company_name' => config('app.name'),
                                'client_name'  => optional($data['order']->client)->name ?? '',
                            ]
                        )
                    );
                }
            ...
            $deliveryMan = $data['order']->delivery_man;
            ...
            if (!empty($contactNumber)) {
                try {
                    ...
                    $message = SMSData('twilio', 'get', null, [
                        'Order ID'          => $data['order']->milisecond,
                        'Delivery_Man Name' => $deliveryMan->name,
                        'status'            => $data['order']->status,
                    ]);
        ...
        case 'courier_picked_up':
            ...
            if ($setting == 1) {
                $dynamicData = [
                    '[order ID]'     => $data['order']->id, // FIX
                    '[status]'       => str_replace('_', ' ', ucfirst($data['order']->status)), // FIX
                    '[Company name]' => config('app.name'),
                    '[client name]'  => isset($data['order']->client) ? $data['order']->client->name : '',
                ];
                ...
                if ($email) {
                    ...
                    Mail::to($email)->send(
                        new SendMail(
                            ...
                            $mailDescription,
                            [
                                'order_id'     => $data['order']->id,     // FIX
                                'status'       => $data['order']->status, // FIX
                                'company_name' => config('app.name'),
                                'client_name'  => isset($data['order']->client) ? $data['order']->client->name : '',
                            ]
                        )
            ...
            // FIX: do NOT stop execution
            if ($smscheck === 0) {
                Log::warning(__('message.sms_permission_denied'));
            }

            $pickupPoint = is_string($data['order']->pickup_point)
                ? json_decode($data['order']->pickup_point, true)
                : ($data['order']->pickup_point ?? []);

            $deliveryPoint = is_string($data['order']->delivery_point)
                ? json_decode($data['order']->delivery_point, true)
                : ($data['order']->delivery_point ?? []);
            ...
            $smsType = $data['order']->sms_type ?? ''; // FIX
            Log::info("Selected SMS Provider: " . $smsType);
            ...
            if (is_array($smsTemplatesPickup) && is_array($smsTemplatesDelivery)) {
                ...
                Log::warning('SMS template invalid'); // FIX (no return)
                break;
            }

            if (!empty($deliveryContact) && !empty($delivery_verification_code)) {
                try {
                    $smsService->sendSMS($deliveryContact, $delivery_verification_code);
                } catch (\Exception $e) {
                    Log::error("Delivery SMS Error: " . $e->getMessage());
                }
            }
            ...
            if (!empty($pickupContact) && !empty($pickup_verification_code)) {
                try {
                    $smsService->sendSMS($pickupContact, $pickup_verification_code);
                } catch (\Exception $e) {
        ...
        case 'completed':
            ...
            if ($order) {
                Payment::where('id', $order->payment_id)->update(['delivery_man_id' => $order->delivery_man_id ]);
            }
        ...
        case 'failed':
            ...
            break;
        case 'pickup_error':
            $data['history_message'] = __('message.pickup_error_history', [
                'id' => $order_id,
                'reason' => $data['order']->reason ?: '-',
            ]);
            $history_data = [
        ...
        case 'return':
            ...
            break;
        case 'reschedule':
            $history_data = [ 'order_id' => $data['order_id'] ];

            $data['history_message'] = __('message.order_rescheduled_by_delivery', ['id' => $order_id]);
            processOrderHistory($data, $history_data);
            $sendTo = ['admin', 'client'];
            ...
            // Agar SMS sending disabled hai, to yahin stop kar do
            if ($smscheck === 0) {
                $errorMessage = __('message.sms_permission_denied');
                Log::warning("SMS Sending Disabled: " . $errorMessage);
                break;
            ...
            if (!empty($contactNumber)) {
                try {
                    $smsType = $data['order']->sms_type ?? 'twilio';
                    Log::info("Selected SMS Provider: " . $smsType);
                    $smsService = app()->make(\App\Services\SmsService::class);

                    $message = SMSData($smsType, 'get', null, [
                        'Order ID' => $data['order']->milisecond,
                        'Customer Name' => $name,
    ...
    $notification_data = [
        ...
        'category' => classifyOrderNotificationCategory($data['history_type']),
    ...
    foreach ($sendTo as $send) {
        ...
        switch ($send) {
            ...
            case 'delivery_man':
                ...
                if ($data['history_type'] == 'courier_assigned') {
                    ...
                    $notification_data['type'] = \App\Services\AppPushService::TYPE_PICKUP_ASSIGNED;
                }
        ...
        if ($user != null) {
            // Item Delivered: AppPushService stores `item_delivered` for OS App — skip duplicate client "completed" row.
            $skipClientCompletedDb = ! empty($data['skip_push'])
                && $send === 'client'
                && ($data['history_type'] ?? '') === 'completed';

            if (! $skipClientCompletedDb) {
                $user->notify(new OrderNotification($notification_data));
            }

            if (empty($data['skip_push'])) {
                if (($notification_data['type'] ?? '') === \App\Services\AppPushService::TYPE_PICKUP_ASSIGNED
                    && $send === 'delivery_man') {
                    try {
                        app(\App\Services\AppPushService::class)->notifyRiderPickupAssigned($data['order'], $user);
                    } catch (\Throwable $e) {
                        Log::warning('pickup assign push failed', ['error' => $e->getMessage()]);
                    }
                } else {
...
function sentOTP_mail($user,$otp_verification_status = 'disable')
{
    $otp = Str::random(6);
...
function prepareUserOsProfileFromRequest($request): array
{
    $osProfile = is_array($request->input('os_profile')) ? $request->input('os_profile') : [];
...
function buildUserAddressFromProfile(array $osProfile): string
{
    return implode(', ', array_filter([
        $osProfile['address_unit'] ?? null,
        $osProfile['street'] ?? null,
        $osProfile['township'] ?? null,
        $osProfile['state_division'] ?? null,
    ]));
}

function registrationEmailFromPhone(string $phone): string
{
    $digits = preg_replace('/\D+/', '', $phone);

    return ($digits ?: 'user').'@pointdelivery.local';
}

function normalizeMyanmarLocalDigits(string $local): string
{
    $number = preg_replace('/\s+/', '', trim($local));

    if ($number === '') {
        return '';
    }

    if (preg_match('/^9\d{7,9}$/', $number)) {
        return $number;
    }
    ...
    if (str_starts_with($number, '959') && strlen($number) > 3) {
        $number = substr($number, 2);
    } elseif (str_starts_with($number, '95')) {
        $remainder = substr($number, 2);
        if (str_starts_with($remainder, '9') && preg_match('/^9\d{7,9}$/', $remainder)) {
            $number = $remainder;
        } elseif (preg_match('/^\d{6,8}$/', $remainder)) {
            $number = '9' . $remainder;
        }
    }

    if (! str_starts_with($number, '9') && preg_match('/^\d{6,8}$/', $number)) {
        $number = '9' . $number;
    }
...
function normalizeContactNumber(?string $phone): string
{
    ...
    if (preg_match('/^[A-Z]{2}\s+(.*)$/i', $phone, $matches)) {
        $phone = trim($matches[1]);
    }

    if (preg_match('/^[A-Z]{2}\+(.*)$/i', $phone, $matches)) {
        $phone = '+' . ltrim($matches[1], '+');
    } elseif (preg_match('/^[A-Z]{2}(\d+)$/i', $phone, $matches)) {
        $phone = '+' . $matches[1];
    }
...
{
    if (isset($data['pickup_point']) && is_array($data['pickup_point'])) {
        if (! empty($data['pickup_point']['contact_number'])) {
            $data['pickup_point']['contact_number'] = normalizeContactNumber($data['pickup_point']['contact_number']);
        }
    }

    if (isset($data['delivery_point']) && is_array($data['delivery_point'])) {
        if (! empty($data['delivery_point']['contact_number'])) {
            $data['delivery_point']['contact_number'] = normalizeContactNumber($data['delivery_point']['contact_number']);
        }

        if (! empty($data['delivery_point']['recipients']) && is_array($data['delivery_point']['recipients'])) {
            foreach ($data['delivery_point']['recipients'] as $index => $recipient) {
                if (! is_array($recipient) || empty($recipient['contact_number'])) {
                    continue;
                }

                $data['delivery_point']['recipients'][$index]['contact_number'] = normalizeContactNumber($recipient['contact_number']);
            }
        }
    }
...
function registrationUsernameFromPhone(string $phone): string
{
...
function buildUserOsProfileFromArray(array $data): array
{
    $osProfile = [
        ...
        'state_division' => $data['state_division'] ?? null,
        'township' => $data['township'] ?? null,
        'street' => $data['street'] ?? null,
    ];

    if (!empty($data['nrc'])) {
        $osProfile['nrc'] = $data['nrc'];
    }

    if (! empty($data['kpay_name'])) {
        $osProfile['kpay_name'] = $data['kpay_name'];
    }

    if (! empty($data['kpay_no'])) {
        $osProfile['kpay_no'] = $data['kpay_no'];
    }

    return array_filter($osProfile, fn ($value) => $value !== null && $value !== '');
}

function createDefaultUserAddressFromRegistration(\App\Models\User $user, ?string $contactNumber = null): void
{
    $address = $user->address;
    if (empty($address) && ! empty($user->os_profile) && is_array($user->os_profile)) {
        $address = buildUserAddressFromProfile($user->os_profile);
    }
    ...
    if (\App\Models\UserAddress::where('user_id', $user->id)->exists()) {
        return;
    }
    ...
    \App\Models\UserAddress::create([
        ...
        'address_type' => __('message.home'),
    ]);
}

function syncUserDefaultAddressLocation(\App\Models\User $user): void
{
    if (empty($user->city_id) && empty($user->country_id)) {
        return;
    }
...
function resolveDispatchPickupPoint(?\App\Models\Order $order): array
{
    if (!$order) {
        return [];
    }

    return is_array($order->pickup_point)
        ? $order->pickup_point
        : (json_decode($order->pickup_point, true) ?? []);
}

function resolveDispatchOsName(?\App\Models\Order $order): string
{
    if (!$order) {
        return '-';
    }

    if (shouldUseClientAsDispatchOs($order)) {
...
/**
 * Exact DeliAmount pay label: Os Pay | Cust Pay | Os Paid.
 *
 * @param  \App\Models\DispatchOrderItem|object|array<string,mixed>  $item
 */
function dispatchDeliPayLabel($item): string
{
    if (is_array($item)) {
        $mode = trim((string) ($item['pickup_pay_mode'] ?? ''));
        $creditTo = strtolower(trim((string) ($item['credit_to'] ?? 'customer')));
    ...
    if ($mode === 'pay_done' || $osPaid > 0) {
        return 'Os Paid';
    }

    if ($mode === 'os_pay' || $creditTo === 'os') {
        return 'Os Pay';
    }
...
/**
 * Format deli amount with exact pay label, e.g. "3,500 (Cust Pay)".
 * Always keeps a space between price and type.
 *
 * @param  \App\Models\DispatchOrderItem|object|array<string,mixed>  $item
 */
function formatDispatchDeliAmount($item, $amount = null): string
{
    if ($amount === null) {
        $amount = is_array($item) ? ($item['deli_amount'] ?? 0) : ($item->deli_amount ?? 0);
    }
    $amount = (float) $amount;
    ...
    $label = trim((string) dispatchDeliPayLabel($item));

    // Price + space + (Type) — never "3,000(Os Pay)"
    return $formatted.' ('.$label.')';
}

/**
 * HTML cell: amount + pay type on one line, column-aligned across rows.
 *
 * @param  \App\Models\DispatchOrderItem|object|array<string,mixed>  $item
 */
function formatDispatchDeliAmountHtml($item, $amount = null): string
{
    if ($amount === null) {
        $amount = is_array($item) ? ($item['deli_amount'] ?? 0) : ($item->deli_amount ?? 0);
    }
    ...
    return formatDeliAmountPartsHtml($formatted, $label);
}

/**
 * HTML for slip/table cells from a display string like "3,000 (Cust Pay)".
 * Always reserves a type column so amounts align with totals.
 */
function formatDeliAmountDisplayHtml(?string $display, $amount = null): string
{
    $raw = trim((string) $display);
    if ($raw !== '' && preg_match('/^([-\d,]+(?:\.\d+)?)\s*\((.+)\)\s*$/u', $raw, $m)) {
        return formatDeliAmountPartsHtml($m[1], trim($m[2]));
    }
    ...
    if ($amount !== null) {
        ...
        return formatDeliAmountPartsHtml($formatted, '');
    }

    if ($raw !== '') {
        return formatDeliAmountPartsHtml($raw, '');
    }

    return formatDeliAmountPartsHtml('0', '');
}

/**
 * Shared markup: price + type on one clean line (never stacked/overlapping).
 * PDFs should prefer formatDeliAmountPartsPlain().
 */
function formatDeliAmountPartsHtml(string $formattedAmount, string $payLabel = ''): string
{
    $typeInner = $payLabel !== ''
        ? '('.e($payLabel).')'
        : '&nbsp;';
    $typeClass = $payLabel !== ''
        ...
        : 'pds-deli-amount-type pds-deli-amount-type--empty';

    // Explicit space between spans so even without CSS they never glue as "3,000(Cust Pay)".
    return '<span class="pds-deli-amount-cell">'
        .'<span class="pds-deli-amount-price">'.e($formattedAmount).'</span>'
        .' '
        .'<span class="'.$typeClass.'">'.$typeInner.'</span>'
        .'</span>';
}

/**
 * Plain text for DomPDF / exports: "3,000 (Cust Pay)".
 */
function formatDeliAmountPartsPlain(string $formattedAmount, string $payLabel = ''): string
{
    $payLabel = trim($payLabel);

    return $payLabel !== ''
        ? $formattedAmount.' ('.$payLabel.')'
        : $formattedAmount;
}

/**
 * Plain display from slip row display string or amount.
 */
function formatDeliAmountDisplayPlain(?string $display, $amount = null): string
{
    $raw = trim((string) $display);
    if ($raw !== '' && preg_match('/^([-\d,]+(?:\.\d+)?)\s*\((.+)\)\s*$/u', $raw, $m)) {
        return formatDeliAmountPartsPlain($m[1], trim($m[2]));
    }
    ...
    if ($amount !== null) {
        ...
        return formatDeliAmountPartsPlain($formatted, '');
    }
...
/**
 * OS slip / list display for OsToPay:
 * - Pay to OS → positive, black
 * - Receive from OS → negative, red
 *
 * @return array{value: float, formatted: string, is_receive: bool}
 */
function formatDispatchOsToPaySlip(float $displayOsToPay): array
{
    if (abs($displayOsToPay) < 0.001) {
        return [
            'value' => 0.0,
            'formatted' => '0',
            'is_receive' => false,
    ...
    $slipValue = -1 * $displayOsToPay;
    $isReceive = $slipValue < 0;

    return [
        'value' => $slipValue,
        'formatted' => number_format($slipValue),
        'is_receive' => $isReceive,
    ];
}

function formatOsClientAddress(?\App\Models\User $os): string
{
    if (! $os) {
        return '-';
    }

    $address = trim((string) ($os->address ?? ''));
    $city = trim((string) ($os->city?->name ?? ''));
    if ($address !== '' && $city !== '' && ! str_contains($address, $city)) {
        $address .= ', '.$city;
    } elseif ($address === '' && $city !== '') {
        $address = $city;
    }
...
function resolveDispatchOsAddress(?\App\Models\Order $order): string
{
    if (!$order) {
        return '-';
    }

    // Match Gate (ကိုယ်တိုင်): always show gate pickup placeholder.
    if ((int) ($order->is_gate_order ?? 0) === 1) {
        return 'ဂိတ်မှ လာယူမည်';
    }

    // Match Shop (ကိုယ်တိုင်): use shop pickup address entered in the app.
    $pickup = resolveDispatchPickupPoint($order);
    $address = $pickup['address'] ?? '';

    return trim((string) $address) !== '' ? trim((string) $address) : '-';
}

function resolveDispatchOsPhone(?\App\Models\Order $order): string
{
    if (!$order) {
        return '-';
    }
    ...
    if (shouldUseClientAsDispatchOs($order)) {
        $phone = normalizeContactNumber(optional($order->client)->contact_number ?? '');

        return $phone !== '' ? $phone : '-';
    }

    $pickup = resolveDispatchPickupPoint($order);
    $phone = normalizeContactNumber($pickup['contact_number'] ?? optional($order->client)->contact_number ?? '');

    return $phone !== '' ? $phone : '-';
}

/**
 * Shop/Gate OS name & phone come from the order-placing User App profile
 * (same for ကိုယ်တိုင် and အခြားလူ). Address follows pickup/gate rules separately.
 */
function shouldUseClientAsDispatchOs(?\App\Models\Order $order): bool
{
    if (!$order) {
        return false;
    }
...
function orderDeliveryRecipients(\App\Models\Order $order): array
{
...
function orderPickupRiderPlaceholder(\App\Models\Order $order): string
{
    if (($order->pickup_vehicle_type ?? 'motorcycle') === 'car') {
        return 'ကားဖြင့်ခေါ်ဆောင်';
    }

    return __('message.select_name', ['select' => __('message.pickup_rider')]);
}

function normalizeDispatchItemSize($value, ?int $default = null): int
{
    if ($value === null || $value === '') {
        return $default !== null ? max(1, min(10, $default)) : 0;
    }
...
/**
 * Size(2)…Size(10): adjust DeliAmount by ±500 per size step.
 * Size(1): returns null — amount is entered manually (no auto 2500).
 */
function adjustDeliAmountForSizeChange($currentAmount, $previousSize, $newSize, int $step = 500): ?float
{
    $next = normalizeDispatchItemSize($newSize, 1);
    if ($next <= 1) {
        return null;
    }

    $prevRaw = ($previousSize === null || $previousSize === '')
        ? 0
        : (int) round((float) $previousSize);
    $prev = max(0, min(10, $prevRaw));
    $current = (float) $currentAmount;

    // No base amount and no prior size — rider must enter DeliAmount on Size(1) first.
    if ($current <= 0 && $prev <= 0) {
        return null;
    }

    // Amount typed on Size(1) without re-tapping Size(1) still counts as baseline size 1.
    $effectivePrev = $prev > 0 ? $prev : 1;

    return (float) max(0, $current + (($next - $effectivePrev) * $step));
}

/**
 * @deprecated Prefer adjustDeliAmountForSizeChange — Size(1) is manual.
 */
function suggestedDeliAmountForSize($size, int $base = 2500, int $step = 500): float
{
    $normalized = normalizeDispatchItemSize($size);
    if ($normalized <= 1) {
        return 0.0;
    }

    return (float) ($base + ($normalized - 1) * $step);
}

function formatDispatchItemSize($value): string
{
...
function expandOrderDeliveryRecipients(\App\Models\Order $order): array
{
    ...
    foreach (orderDeliveryRecipients($order) as $recipient) {
        $count = max(1, (int) ($recipient['parcel_count'] ?? 1));
...
function resolveDispatchItemCustomerPrefill(\App\Models\Order $order, ?\App\Models\DispatchOrderItem $item = null, bool $isEdit = false): array
{
    if ($isEdit && $item) {
        return [
            'customer_name' => $item->customer_name,
            'customer_phone' => normalizeContactNumber($item->customer_phone),
    ...
    $flat = expandOrderDeliveryRecipients($order);
    if (! empty($flat)) {
        $index = $order->dispatchItems()->where('status', 'collected')->count();
        // Extra parcels beyond recipient list stay blank — do not reuse the last customer.
        ...
        return [
            ...
            'customer_phone' => normalizeContactNumber($recipient['contact_number'] ?? ''),
...
function resolveDispatchOrderType(\App\Models\Order $order): array
{
    $isSelf = (int) ($order->is_self_order ?? 1) === 1;

    if ((int) ($order->is_shop_order ?? 0) === 1) {
        return [
            'key' => 'shop',
            'label' => $isSelf ? 'ဆိုင် (မိမိဆီသို့)' : 'ဆိုင် (အခြားလူဆီသို့)',
            'class' => 'pds-order-type-shop',
        ];
    }
    ...
    if ((int) ($order->is_gate_order ?? 0) === 1) {
        return [
            'key' => 'gate',
            'label' => $isSelf ? 'ဂိတ် (မိမိဆီသို့)' : 'ဂိတ် (အခြားလူဆီသို့)',
            'class' => 'pds-order-type-gate',
        ];
    }

    if ((int) ($order->is_photo_order ?? 0) === 1) {
        return ['key' => 'photo', 'label' => __('message.photo_order'), 'class' => 'pds-order-type-photo'];
    }

    if ((int) ($order->is_text_order ?? 0) === 1) {
        return ['key' => 'text', 'label' => __('message.text_order'), 'class' => 'pds-order-type-text'];
    }

    return ['key' => 'standard', 'label' => __('message.standard_order'), 'class' => 'pds-order-type-standard'];
}

function collectGatePassImages(\App\Models\Order $order): array
{
    $images = [];
    $order->loadMissing('profofPictures');

    foreach ($order->profofPictures as $picture) {
        if (($picture->type ?? '') !== 'gate_pass') {
            continue;
        }
